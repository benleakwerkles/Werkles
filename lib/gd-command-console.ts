/**
 * GD Command Console — intent-first governor (preview/local).
 * Ben states what he wants; GD classifies topic and routes cousins by role.
 * No external AI. No persistence. Aligns with gd-intent-router config.
 */

import cousinAssignment from "@/foreman/gd-intent-router/cousin-assignment.json";
import missionRegistry from "@/foreman/gd-intent-router/mission-classes.json";

export type GdRiskLevel = "LOW" | "MEDIUM" | "HIGH" | "BLOCKED";

export type GdVerdict = "DRAFT_PACKET_ONLY" | "HUMAN_REVIEW_REQUIRED" | "NO_GO";

export type GdRoutedCousin = {
  id: string;
  label: string;
  platform: string;
  seat: string;
...
export type GdClassificationResult = {
  verdict: GdVerdict;
  missionClass: string;
  missionLabel: string;
  missionDescription: string;
  routedCrew: GdRoutedCousin[];
  recipient: string;
  recipients: string[];
...
type MissionDef = {
  ...
  recipients?: string[];
  synthesisLead?: string | null;
  hgApprovalLevel: string;
  dispatchClass: string;
  cousinLenses?: Record<string, string>;
};

const COUSIN_META = cousinAssignment.cousins as Record<
  string,
  { label: string; platform: string; seat: string }
>;
...
const HG_NOTES: Record<string, string> = {
...
const HARD_STOP_RULES: { id: string; pattern: RegExp; stop: string }[] = [
...
const HARD_STOP_NEGATIONS: Record<string, RegExp[]> = {
  deploy: [
...
const UNCLASSIFIED_MISSION = {
  ...
  recipients: ["OPERATOR"] as string[],
  synthesisLead: null,
  hgApprovalLevel: "HG_OPERATOR",
  dispatchClass: "NONE"
};

function resolveMission(missionClass: string): { key: string; def: ResolvedMissionDef } {
  ...
  const registry = missionRegistry.missionClasses as Record<string, MissionDef>;
  const def = registry[missionClass];
  if (!def) return { key: "UNCLASSIFIED", def: UNCLASSIFIED_MISSION };

  const assignments = cousinAssignment.assignments as Record<string, string[]>;
...
function shouldApplyHardStop(text: string, rule: (typeof HARD_STOP_RULES)[number]) {
...
function detectHardStops(text: string) {
...
function formatRecipients(keys: string[]) {
  return keys.map((k) => COUSIN_LABELS[k] ?? k).join(" · ");
}

function riskFromHg(level: string, fallback: GdRiskLevel): GdRiskLevel {
...
function buildNextAction(
  ...
  if (missionClass === "THREAD_REFRESH_PACKET") {
    return "Governor routes cockpit-direct. When ready: npm run gd:thread-refresh (or Foreman GimpDash Generate).";
  }
...
export function classifyGdCommand(input: string): GdClassificationResult {
  ...
  let ruleRisk: GdRiskLevel = "MEDIUM";
  ...
  let verdict: GdVerdict = "DRAFT_PACKET_ONLY";
...
export function formatGdCommandVerdict(result: GdClassificationResult): string {
...
export function isGdCommandConsoleEnabled() {
  if (process.env.NODE_ENV === "production" && process.env.GD_COMMAND_CONSOLE !== "1") {
    return false;
  }
  return process.env.NODE_ENV === "development" || process.env.GD_COMMAND_CONSOLE === "1";
}
