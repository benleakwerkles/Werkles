...
export type HarveySnapshotMachine = {
  ...
  heartbeat_error?: "HEARTBEAT_TIMESTAMP_INVALID" | "HEARTBEAT_IDENTITY_INVALID";
  latest_command: null | {
    command_id: string;
...
const COMMAND_ACTIONS = new Set(["OPEN_URL", "PING", "KNOCK"]);
...
function connectivityForHeartbeat(machine: HarveyMachine, heartbeat: unknown, nowMs: number) {
  if (!heartbeat || typeof heartbeat !== "object") return { connectivity: "DISCONNECTED" as const, observedAt: null, ageMs: null };
  ...
  if (record.machine !== machine || record.hostname !== identity.hostname || record.agent_id !== identity.agent_id) {
    return { connectivity: "DISCONNECTED" as const, observedAt: observedAt || null, ageMs: null, error: "HEARTBEAT_IDENTITY_INVALID" as const };
  }
  const timestamp = parseTimestamp(observedAt);
  if (timestamp === null || timestamp > nowMs) {
    return { connectivity: "DISCONNECTED" as const, observedAt: observedAt || null, ageMs: null, error: "HEARTBEAT_TIMESTAMP_INVALID" as const };
  }
  const ageMs = nowMs - timestamp;
  const connectivity: HarveyConnectivity = ageMs <= LIVE_MAX_MS ? "LIVE" : ageMs <= STALE_MAX_MS ? "STALE" : "DISCONNECTED";
  return { connectivity, observedAt, ageMs };
}
...
function normalizeWorkstreams(report: WorkstreamReport | undefined, commands: ReturnType<typeof normalizeCommands>, nowMs: number, errors: string[]) {
  ...
  return raw
    ...
    .map<HarveySnapshotWorkstream>((stream) => {
      ...
      const associated = commands.filter(({ command }) => command.workstream_id === workstreamId && (command.status === "COMPLETED" || command.status === "BLOCKER"));
      const latest = associated[0];
      ...
      const executionStatus: HarveySnapshotWorkstream["execution_status"] = !latest
        ? "UNPROVEN"
        : !latest.evidence.valid || !terminalReceipt
          ? "EVIDENCE_INVALID"
      ...
      return {
        ...
        latest_receipt_at: terminalReceipt?.observed_at ?? null
      };
    })
