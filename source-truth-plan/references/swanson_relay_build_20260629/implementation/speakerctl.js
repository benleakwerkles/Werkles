...
const SOURCE_TRUTH_POINTERS_PATH = path.join(SPEAKER_ROOT, "db", "source_truth_pointers.json");
const SOURCE_TRUTH_PLAN_ROOT = process.env.SOURCE_TRUTH_PLAN_ROOT || "C:\\wt\\stbook\\source-truth-plan";
const SOURCE_TRUTH_PLAN_URL = process.env.SOURCE_TRUTH_PLAN_URL || "https://github.com/benleakwerkles/Werkles1/tree/main/source-truth-plan";
...
function readSourceTruthPointers() {
  if (!fs.existsSync(SOURCE_TRUTH_POINTERS_PATH)) {
    return {
      exists: false,
      sha256: null,
      excerpt: "MISSING",
  ...
  const raw = fs.readFileSync(SOURCE_TRUTH_POINTERS_PATH);
  return {
    exists: true,
    byte_count: raw.length,
    sha256: sha256Buffer(raw),
    excerpt: raw.toString("utf8").trim().slice(0, 900),
  };
}
...
function renderSourceTruthBrainboot(profile) {
  ...
  const sourceTruthPointers = readSourceTruthPointers();
  ...
  const lines = [
    ...
    "- Treat GitHub main/source-truth-plan as the current source-truth plan unless a newer receipt supersedes it.",
    "- Treat references/betsy_desktop_nerdkle_the_book as raw source material and editing reference, not as a replacement source-truth plan.",
  ...
  lines.push(`- C:\\speaker\\db\\source_truth_pointers.json: ${sourceTruthPointers.exists ? `FOUND sha256=${sourceTruthPointers.sha256}` : "MISSING"}`);
  ...
  if (sourceTruthPointers.exists) {
    lines.push("");
    lines.push("### C:\\speaker\\db\\source_truth_pointers.json");
    lines.push("");
    lines.push("```json");
    lines.push(sourceTruthPointers.excerpt);
    lines.push("```");
  }
