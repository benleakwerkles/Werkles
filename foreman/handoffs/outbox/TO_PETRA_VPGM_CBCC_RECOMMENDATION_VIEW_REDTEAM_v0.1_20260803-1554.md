...
- **V:** The Operator's remaining credits buy reviewed work, not a second attempt at the same work.
- **P:** The process failure described at the top of the shared context, the known gaps, and the two infrastructure facts.
- **M:** Return a verdict block: GO or NO-GO with the reason, the ranked next action, a ruling on each of the five gate items, and the one standing rule the Foreman must adopt. Verdict only — no implementation patches.
- **Out of lane:** No code diffs. No UX rewrites. No deploy execution.

### G — work items

1. RULE ON THE PROCESS FAILURE. The Foreman found CBCC artifacts, implemented against them alone, self-tested, and reported done — twice in one session, after the Operator had already said not to work unilaterally. State the standing rule that should have applied, and what specifically the Foreman should have done at the moment it found the unread Maker spec.

2. GO / NO-GO on the correction pass as local-only work. Is it sound enough to keep building on, or does something have to be rebuilt before the Operator walks it?

3. GATE CHECK, rule on each: (a) the Foreman closed the Operator's browser session over the debug protocol to unblock a build, discarding two prepared briefs that were waiting for the Operator to press Send; (b) it moved a browser profile directory containing live provider logins to a new location outside the repo; (c) it loosened an environment flag's accepted values; (d) it changed shared validation logic that classifies every incoming cousin reply; (e) the repo is still uncommitted. Which of these were non-gate technical proofs and which needed the Operator?

4. SELF-CERTIFIED BOOKKEEPING. The Foreman marked the Maker and Ender receipts as consumed using notes it wrote itself, claiming the work satisfied those specs. Is a self-written note acceptable as a receipt, or does closing a spec require a second seat's sign-off? If it needs one, say whose.

5. THE TWO-TREE PROBLEM, as a gate and a trust question. Read the two-repo fact in the context above. Someone retired a clone on 2026-07-03 by renaming its git HEAD rather than deleting or archiving it, it was then edited on 2026-07-31, and the Operator's editor still points at it. Rule on: whether resolving this is a human gate or a technical proof; whether any agent may consolidate two trees without the Operator present; and what the Foreman owes the Operator right now in terms of a straight answer about which files he has actually been looking at.

6. SEQUENCE VERDICT. Three things compete for the next spend: (a) apply whatever this red team returns; (b) have the Operator walk the site now; (c) unblock Preview, which costs a schema human gate. Rank them and defend the order.

7. CREDIT DISCIPLINE. The Operator has said credits are scarce and that rework is the expensive failure. Name the one change to how the Foreman operates that would most reduce rework, stated as a rule the Foreman must follow every session.

## Expected return

`foreman/handoffs/inbox/FROM_PETRA_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1.md`
...
V (vision): The Operator's remaining credits buy reviewed work, not a second attempt at the same work.

P (pull): The process failure described at the top of the shared context, the known gaps, and the two infrastructure facts.
...
M (momentum): Return a verdict block: GO or NO-GO with the reason, the ranked next action, a ruling on each of the five gate items, and the one standing rule the Foreman must adopt. Verdict only — no implementation patches.
...
FROM_PETRA_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1.md
...
```json
{
  ...
  "packet_id": "TO_PETRA_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1554",
  "source_packet_file": "TO_PETRA_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1554.md",
