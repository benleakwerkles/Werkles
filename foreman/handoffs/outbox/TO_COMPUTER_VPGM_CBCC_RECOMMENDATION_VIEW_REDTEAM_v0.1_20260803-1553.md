...
- **V:** Werkles presents uncertainty the way careful companies do, and the recommendation framing does not walk into an advice boundary.
- **P:** The band scheme, the single-verdict framing, the disabled action, and the measured font sizes.
- **M:** Return cited findings only, with sources named. Flag anything you cannot source rather than filling the gap. Do not issue new Werkles doctrine.
- **Out of lane:** No deploy recommendations. No unsourced claims.

### G — work items

1. SCORE VERSUS BAND, with citations. Do comparable products present a single recommendation with qualitative strength labels rather than a numeric score? Look at credit and lending decisions, recruiting and candidate matching, and dating or professional matchmaking. Cite real examples and the exact labels used, and note where showing the number is considered better practice for auditability.

2. IS HIDING THE NUMBER MORE HONEST OR LESS? The Foreman removed a 0-92 integer from the interface and kept it server-side. Cite anything from explainability, algorithmic transparency, or consumer-protection guidance that bears on hiding a score you are still acting on. Include the counter-argument.

3. ADVICE BOUNDARY. The surface tells a small-business owner 'build proof before asking for a Backer' and 'find an Operator first', in a product that also sells identity and funds verification. Cite anything relevant to a non-advisor presenting recommendations that touch financing decisions, and say plainly whether the current framing and the standing trust note are enough.

4. UNREACHABLE CONFIDENCE LEVEL. The interface reports HIGH, MEDIUM, or LOW confidence, but HIGH cannot currently occur. Is there any guidance or precedent on presenting confidence scales with unreachable values? Flag if you cannot source it.

5. TYPOGRAPHY MINIMUMS, with citations. Measured body copy is 12.2px bold, strength band labels 11.2px, and one explanation 13.1px, all at contrast of 11.6:1 or better. Cite the accepted minimum body text size guidance and whether high contrast offsets small size. State whether these sizes pass or fail.

## Expected return

`foreman/handoffs/inbox/FROM_COMPUTER_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1.md`
...
V (vision): Werkles presents uncertainty the way careful companies do, and the recommendation framing does not walk into an advice boundary.

P (pull): The band scheme, the single-verdict framing, the disabled action, and the measured font sizes.
...
M (momentum): Return cited findings only, with sources named. Flag anything you cannot source rather than filling the gap. Do not issue new Werkles doctrine.
...
FROM_COMPUTER_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1.md
...
```json
{
  ...
  "source_packet_id": "TO_COMPUTER_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1553",
  "source_packet_file": "TO_COMPUTER_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1553.md",
...
```json
{
  "schemaVersion": "aeye-crew-relay/v0.1",
  "cousin": "COMPUTER",
  "generated_at": "2026-08-03T15:53:16.268Z",
  "currentStateHash": "78e580fb3019107585768920e8d2f5fc289e6533f4b1716081bea719d772242a",
  "nextActionHash": "086ce991f8e566bbd82726d2d6de2d0e14818bd78c80ced82ef4080bc587466a",
  ...
  "packet_id": "TO_COMPUTER_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1553",
  "source_packet_file": "TO_COMPUTER_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1553.md",
