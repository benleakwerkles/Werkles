...
- **V:** The walkthrough machine behaves predictably, and no local tooling can silently break the build again.
- **P:** The two infrastructure facts, the environment flags, and the pipeline classification change.
- **M:** Return the ranked guards and options with the gate each one triggers. Concrete commands or config are welcome; do not apply SQL, deploy, enter secrets, or click provider buttons.
- **Out of lane:** No UX or copy opinions. No trust or compliance verdicts — that is Bean.

### G — work items

1. THE BUILD WAS BROKEN AND NOBODY KNEW. A browser profile directory living inside the repo held an OS lock on its cookie database; the production build globbed the repo root and died with EBUSY. It failed on every attempt while the browser ran. The fix was moving the profile outside the repo. Is that sufficient, or does the build also need an explicit ignore rule, a preflight check, or a repo-hygiene rule that no runtime state may live in the tree? Give the concrete guard.

2. ORPHANED SERVER PROCESS. The local walkthrough server was started through a package-manager wrapper. The wrapper died after seven hours while the actual server kept the port, so a restart attempt failed on address-in-use and the site was being served by a process no shell owned. What is the correct local run pattern so a walkthrough is never served by an orphan, and how should health be checked without port-scanning tools?

3. ENVIRONMENT FLAG SPRAWL. Three flags now gate local behavior: one exposing a synthetic fleet to the browser bundle, one enabling that fleet server-side, one opening personal recommendations. One of them silently served the wrong content when given '1' instead of 'true'. Recommend consolidation, and state which of these must be impossible to enable on Production and how that is enforced rather than intended.

4. UNHASHED RECEIPTS. Cousin replies from a second router are now accepted without any cockpit hash, so staleness cannot be checked; they carry an advisory warning instead. Is accepting unhashed receipts an acceptable infra posture, or should they be stamped on arrival?

5. THE TWO-TREE PROBLEM — treat this as your first item. Read the two-repo fact in the context above. A retired snapshot with a renamed git HEAD is sitting where the editor's workspace root points, it received a stylesheet edit on 2026-07-31, three weeks after it was retired, and it cannot be inspected with git because git does not recognise it. Recent unexplained symptoms on this project include pages not loading, wrong images appearing in slots, and copy that did not match what was written. Give me: (a) how an edit reaches a retired tree at all, and whether an editor pointed at it plus an agent running in the live tree explains those symptoms; (b) the safe order of operations to resolve it without losing anything, given the retired tree may hold work that was never committed anywhere; (c) how to tell definitively whether that 2026-07-31 stylesheet contains unique work or is just an older copy; (d) the guard that makes a second tree impossible to edit by accident. Do not propose deleting anything — the Operator decides that.

6. PREVIEW, briefly. Intake writes JSON to the local filesystem, which is read-only on Vercel. Enumerate options with the human gate each triggers and whether it is reversible, and name the cheapest reversible path.

## Expected return

`foreman/handoffs/inbox/FROM_SKYBRO_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1.md`
...
V (vision): The walkthrough machine behaves predictably, and no local tooling can silently break the build again.

P (pull): The two infrastructure facts, the environment flags, and the pipeline classification change.
...
M (momentum): Return the ranked guards and options with the gate each one triggers. Concrete commands or config are welcome; do not apply SQL, deploy, enter secrets, or click provider buttons.
...
FROM_SKYBRO_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1.md
...
```json
{
  "schemaVersion": "aeye-crew-relay/v0.1",
  "cousin": "SKYBRO",
  "generated_at": "2026-08-03T15:54:52.458Z",
  "currentStateHash": "78e580fb3019107585768920e8d2f5fc289e6533f4b1716081bea719d772242a",
  "nextActionHash": "086ce991f8e566bbd82726d2d6de2d0e14818bd78c80ced82ef4080bc587466a",
  ...
  "packet_id": "TO_SKYBRO_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1554",
  "source_packet_file": "TO_SKYBRO_VPGM_CBCC_RECOMMENDATION_VIEW_REDTEAM_v0.1_20260803-1554.md",
