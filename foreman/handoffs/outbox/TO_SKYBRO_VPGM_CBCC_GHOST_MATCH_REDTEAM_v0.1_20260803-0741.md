...
- **V:** Preview works without asking the Operator to approve a schema change he does not need yet.
- **P:** The Preview blocker in the known gaps above.
- **M:** Return the ranked options with gates named. Do not apply SQL, deploy, enter secrets, or click provider buttons.
- **Out of lane:** No UX or copy opinions. No trust/compliance verdicts — that is Bean.

### G — work items

1. Intake persistence on Vercel Preview. Today intake writes JSON files to the repo filesystem, which is read-only on Vercel. Enumerate the options — Supabase table, Vercel KV/Blob, edge config, in-memory with honest TTL disclosure — with the human gate each one triggers and whether it is reversible.

2. Cheapest reversible path. Which option gets a working Preview walkthrough with the smallest gate and the least lock-in?

3. Environment safety. The Ghost Fleet is closed on Production by an env check (VERCEL_ENV === 'production' returns false). Is an env check sufficient, or should the fleet data be excluded from the Production build entirely?

## Expected return

`foreman/handoffs/inbox/FROM_SKYBRO_CBCC_GHOST_MATCH_REDTEAM_v0.1.md`
...
V (vision): Preview works without asking the Operator to approve a schema change he does not need yet.

P (pull): The Preview blocker in the known gaps above.
...
M (momentum): Return the ranked options with gates named. Do not apply SQL, deploy, enter secrets, or click provider buttons.
...
FROM_SKYBRO_CBCC_GHOST_MATCH_REDTEAM_v0.1.md
...
```json
{
  ...
  "source_packet_id": "TO_SKYBRO_VPGM_CBCC_GHOST_MATCH_REDTEAM_v0.1_20260803-0741",
  "source_packet_file": "TO_SKYBRO_VPGM_CBCC_GHOST_MATCH_REDTEAM_v0.1_20260803-0741.md",
...
```json
{
  ...
  "packet_id": "TO_SKYBRO_VPGM_CBCC_GHOST_MATCH_REDTEAM_v0.1_20260803-0741",
  "source_packet_file": "TO_SKYBRO_VPGM_CBCC_GHOST_MATCH_REDTEAM_v0.1_20260803-0741.md",
