...
|-------|-------|
| **Cousin ID** | BEAN |
| **Seat** | Hostile audit cousin |
| **Platform** | DeepSeek |
| **Edge tab** | #4 → https://chat.deepseek.com/ |
| **Lane** | Trust, compliance, hardening audits — not deploy execution. |
| **Relay job (this command)** | Acknowledge audit lane; flag overreach and human gates; do not recommend deploy/push. |
...
| Response inbox | `foreman/handoffs/inbox/` as `FROM_BEAN_*.md` |
...
## Required response (Bean)

Reply with a `FROM_BEAN_RELAY_ROLE_ACK_*.md` saved to inbox (Ben will paste your reply into a file, or you output markdown Ben saves).

Include **Relay metadata** JSON with all `REQUIRED_RESPONSE_FIELDS` from schema plus:

- `VERDICT`: one line — e.g. `ROLE_ACK — BEAN lane understood`
- `CONFIDENCE`: `HIGH` or `LOW`
- `UNKNOWNS`: `none` or list; say **outside my lane** if unsure
- `source_packet_id`: `TO_BEAN_RELAY_ROLE_AWARENESS_SYNC_v0.1_20260531-0355`
- `source_packet_file`: `TO_BEAN_RELAY_ROLE_AWARENESS_SYNC_v0.1_20260531-0355.md`
...
```json
{
  ...
  "cousin": "BEAN",
  "generated_at": "2026-05-31T03:55:07.162Z",
  ...
  "packet_id": "TO_BEAN_RELAY_ROLE_AWARENESS_SYNC_v0.1_20260531-0355",
  "source_packet_file": "TO_BEAN_RELAY_ROLE_AWARENESS_SYNC_v0.1_20260531-0355.md",
