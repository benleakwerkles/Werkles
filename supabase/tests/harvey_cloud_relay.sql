...
begin
  ...
  select delivery_id, command_id into v_delivery, v_command from harvey_claim_a;

  begin
    perform public.harvey_record_delivery_receipt(v_delivery, 'doss:test-a', 'REPLIED', 'Invalid direct reply.', null, '{}'::jsonb);
  ...
  perform public.harvey_record_delivery_receipt(v_delivery, 'doss:test-a', 'WORKING', 'Receiver is working.', null, '{}'::jsonb);
  perform public.harvey_record_delivery_receipt(v_delivery, 'doss:test-a', 'COMPLETED', 'Receiver completed.', null, '{}'::jsonb);
...
declare
  ...
  v_status text;
begin
  select delivery_id, command_id into v_delivery, v_command from harvey_claim_b;
  perform public.harvey_record_delivery_receipt(
    v_delivery, 'doss:test-b', 'BLOCKED', 'Receiver blocked.', 'Needs a route.', '{"error_code":"TEST_BLOCKER"}'::jsonb
  );
  select status into v_status from public.harvey_relay_commands where command_id = v_command;
...
declare
  ...
  v_status text;
begin
  select delivery_id, command_id into v_delivery, v_command from harvey_claim_wait_mix;
  perform public.harvey_record_delivery_receipt(v_delivery, 'doss:test-a', 'COMPLETED', 'Receiver completed.', null, '{}'::jsonb);
  select status into v_status from public.harvey_relay_commands where command_id = v_command;
...
begin
  ...
  select delivery_id into v_delivery from harvey_claim_reclaimed;
...
begin
  ...
  if has_function_privilege('service_role', 'public.harvey_claim_deliveries(text,text,integer,integer)', 'execute') then raise exception 'HARVEY_TEST_SERVICE_CLAIM_NOT_LOCKED'; end if;
  if has_function_privilege('service_role', 'public.harvey_record_delivery_receipt(uuid,text,text,text,text,jsonb)', 'execute') then raise exception 'HARVEY_TEST_SERVICE_RECEIPT_NOT_LOCKED'; end if;
